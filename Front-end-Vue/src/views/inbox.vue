    ...
    <div>
     <el-row :gutter="24" v-if="dialogVisible">
      <el-col :xs="24" :sm="21" :md="21" :lg="21" :xl="21">
        <div class="">
          ...
          <el-table :data="allInboxMail" style="width: 100%; background-color:#A9B8C0">
     ...
     <el-row v-if="!dialogVisible">
         ...
         <el-row style="background-color: aliceblue; height:50vh">
             ...
             <el-col :md="18">
               <h2 style="margin-left:5%">{{this.viewdata.sender}}</h2>
...
export default {
    ...
    computed: mapGetters(["allInboxMail","mailSended"]),
    created(){
      this.getMail();
    },
    data(){
    ...
    methods: {
       ...mapActions(["getInboxMail","deleteInboxMail","Compose_Send"]),

      getMail(){
            const id =sessionStorage.getItem("Email");
            this.getInboxMail(id).then(
              data=>{
                console.log(this.allInboxMail)
                console.log(data)
      ...
      handleDelete(index, row) {
         this.$confirm('Do you want Delete Mail. Continue?', 'Warning', {
          ...
          this.deleteInboxMail(row.id).then(data=>{
            this.getMail();
            this.garbage = data;
            this.$message({
      ...
      DeleteMailInView(){
          ...
          this.deleteInboxMail(this.viewdata.id).then(data=>{
            this.getMail();
            this.garbage = data;
